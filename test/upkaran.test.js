...
contract("~upkaran works", function (accounts) {
  ...
  describe("handles ERC20s correctly", function () {
    it("when transferFrom is called by approver itself", async function () {
      ...
      let calls = [];
      const transferFromTosomeoneElseCall = formateCall(
        this.erc20.address,
        this.erc20.contract.methods.transferFrom(owner, someoneElse, amount).encodeABI(),
      );
    ...
    it("reverts when transferFrom is called by someone else", async function () {
      ...
      const expectedError =
        "Returned error: VM Exception while processing transaction: revert Upkaran/transferFrom-not-allowed -- Reason given: Upkaran/transferFrom-not-allowed.";
      try {
  ...
  describe("handles NFTs correctly", function () {
    it("when data is given (data = should transfer NFT back)", async function () {
      let calls = [];
      const transferNFTBackCall = formateCall(
        this.nft.address,
        this.nft.contract.methods.safeTransferFrom(this.upkaran.address, owner, tokenId).encodeABI(),
      );
  ...
  describe("handles ERC777 correctly", function () {
    it("when data is given (data = should transfer ERC777 back)", async function () {
      let calls = [];
      const transferERC777BackCall = formateCall(
        this.erc777Token.address,
        this.erc777Token.contract.methods.transfer(owner, amount).encodeABI(),
      );
  ...
  describe("handles ERC1155 correctly", function () {
    describe("single transfer", function () {
      it("when data is given (data = should transfer ERC1155 back)", async function () {
        let calls = [];
        const transferERC1155BackCall = formateCall(
          this.erc1155Token.address,
          this.erc1155Token.contract.methods
            .safeTransferFrom(this.upkaran.address, owner, tokenId, amount, emptyData)
            .encodeABI(),
        );
      ...
      it("when data is not given", async function () {
        ...
        expect((await this.erc1155Token.balanceOf(this.upkaran.address, tokenId)).toString()).to.be.equal(
          amount.toString(),
        );
      });
    });
    describe("batch transfer", function () {
      it("when data is given (data = should transfer ERC1155 back)", async function () {
        ...
        const transferERC1155BackCall = formateCall(
          this.erc1155Token.address,
          this.erc1155Token.contract.methods
            .safeBatchTransferFrom(this.upkaran.address, owner, tokenIds, amounts, emptyData)
            .encodeABI(),
        );
        calls.push(transferERC1155BackCall);
        const data = convertCallsToBytes(calls);
        await this.erc1155Token.safeBatchTransferFrom(owner, this.upkaran.address, tokenIds, amounts, data);
        expect((await this.erc1155Token.balanceOfBatch([owner, owner], tokenIds)).toString()).to.be.equal(
          amounts.toString(),
        );
      });
      it("when data is not given", async function () {
        ...
        expect(
          (await this.erc1155Token.balanceOfBatch([this.upkaran.address, this.upkaran.address], tokenIds)).toString(),
        ).to.be.equal(amounts.toString());
      });
    });
  });
});
