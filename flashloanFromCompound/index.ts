...
const takeFlashLoanFromCompund = async function () {
    ...
    let calls = [];
    const approveWETHToERC3156Lender = formateCall(weth.address, erc20.interface.encodeFunctionData("approve", [erc3156Lender.address, amount.add(flashFees)]));
    ...
    //Enter markets 
    const enterMarketsCall = formateCall(comptroller.address, comptroller.interface.encodeFunctionData("enterMarkets", [[cDAI.address, cETH.address]]))
    ...
    const approveDAItocDAI = formateCall(dai.address, dai.interface.encodeFunctionData("approve", [cDAI.address, amountOfDaiToBorrow]))
    calls.push(approveDAItocDAI)

    //repay DAI
    const repayDAICall = formateCall(cDAI.address, cDAI.interface.encodeFunctionData("repayBorrow", [amountOfDaiToBorrow]))
    ...
    //convert ETH to WETH
    const convertETHToWETHCall = formateCall(weth.address, weth.interface.encodeFunctionData("deposit"), amount.toString())
